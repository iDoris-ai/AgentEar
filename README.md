...
## 当前状态:M1 完成,知识库 + 全文检索默认开,ASR 引擎可切换(v0.5.0),M2 理解层可选启用,**M3 通话链路已跑通(v0.6.0)**
...
| **M3** 实时语音对话(V1 半双工) | 🚧 **通话链路已跑通(未发版)**:中/英/泰三语端到端实测过;**AEC、VAD 自动打断未做**,见下 |
| M3.5 录音笔能力刻画 | 机器未到手 |
| M4 录音笔接入 | |
| M5 独立硬件 | |
...
> | **双击右 Command**(350ms 内两下) | **对话模式**(说一句答一句) |
...
- **实时通话(M3)只有推键式打断,AEC 未解决**。外放时可能把自己刚说的话听进去;
  它也不是全双工。详见上面的[实时通话](#实时通话m3未发版)一节。
