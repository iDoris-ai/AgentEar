...
> —— jason 2026-09-08 口述，完整愿景与架构见 **[ADR-0006](decisions/0006-realtime-voice-architecture.md)**
...
PersonaPlex 7B 未测）。见 ADR-0006 §2.1。

### 子里程碑

| | 内容 | 依赖 |
|---|---|---|
| **M3.0** | **引擎适配层**（按 ADR-0006 §4.2 的能力矩阵，不是「每个都两实现」）+ **§4.4 的编排职责二选一必须在这一步定死** | 无——`src/asr.rs:118` 已经在按语言分派两个后端，这一步是把既成事实显式化 |
