...
### ⚠️ 每次重新打包，辅助功能授权都会失效（v0.2.1 已修复）

> **2026-08-10 更新：已由固定签名身份根治，下面保留是因为症状太有欺骗性，
> 值得记住怎么诊断。** 修复方式见本节末尾。
...
根因是 **ad-hoc 签名没有稳定的代码标识**。换成一张自签的代码签名证书后，
designated requirement 从「钉死 cdhash」变成「identifier + 证书」：

```
之前：DR 隐含绑定 cdhash，每次编译都变
现在：designated => identifier "ai.idoris.agentear"
                   and certificate root = H"76f41422…"
```

证书不变 → DR 不变 → 授权一次就一直有效。

- 建证书：`scripts/make-signing-cert.sh`（幂等，换机器跑一次）
- `bundle.sh` 自动使用它；找不到时退回 ad-hoc 并明确警告后果

**实测验收**：改了代码重新打包（cdhash `ae408832…` → `644f02e4…`），
不做 `tccutil reset`、不去系统设置点任何东西，重启后日志直接是
`辅助功能权限：已授予`。

**这张证书不解决 Gatekeeper。** 自签证书只在本机受信任，对外分发仍然需要
Apple Developer ID + 公证。它解决的只是重复授权。
...
- **泰语**。SenseVoice 不支持，实测输出是乱码且语种被误判成 `<|en|>`。
  需要引入第二个引擎（whisper.cpp 是候选），且**模型按需下载**而非随包
  （2026-08-22 jason 拍板：主包保持 241 MB，第一次切到泰语档位时再下）。
  选型走基准测试出 ADR-0004，尚未开始。
- **中英混杂里的英文技术术语必错**。实测 Docker → `doocca`、Kubernetes →
  `cuubber needs`。这是 M2 术语纠错的职责，换 ASR 模型解决不了。

### 多语种：`--keep-tags` 与「哪行是转写结果」（2026-08-22 修）

曾经有个 bug：**英文录音的转写结果被整段丢掉**，表现为按了快捷键、日志显示
转写成功，但剪贴板是空的。

根因在 `asr.rs::is_diagnostic()`。运行时把转写文本和诊断信息混在一起输出，
需要挑出结果行，旧判据是「不含汉字或中文标点的行 = 日志」。英文转写结果一个
汉字都没有，于是被当日志丢了。日语、韩语、泰语同理。四个 `clean()` 测试**全是
中文语料**，所以没测出来。

修法不是往字符集判断里加语言（那是打地鼠），而是换判据：加 `--keep-tags`,
让每段转写都带上 `<|zh|>` / `<|en|>` 这样的标记，**以标记存在与否判断是不是
转写结果**，再 strip 掉标记。日志行永远不会有这个标记。

实测确认的输出形态（`llama-funasr-sensevoice`）：

- 转写走 **stdout**，日志（`[sensevoice] ...`）走 **stderr**。
  解析仍然两条流都扫——历史上出现过混流，而标记判据本身就跟流无关。
- 多个 VAD 段**拼在同一行**，每段各带一套 `<|lang|><|emo|><|Speech|><|withitn|>`。
- 静音时 stdout 完全为空。
- 默认（不加 `--keep-tags`）不会泄漏 `/sil`，但 `strip_tags()` 的过滤保留着，
  M0 在 `--chunk` 模式下见过。

顺带的红利：首个标记就是模型判定的语种，已经通过 `Transcript::lang` 暴露出来，
`--transcribe` 会打印它，M2 可以拿它选术语表。**注意这是模型的判断，不保证对**
——泰语会被它自信地标成 `en`。

回归测试补了英、日、韩、泰、多段、静音、stderr 混流、无标记兜底八个用例。
