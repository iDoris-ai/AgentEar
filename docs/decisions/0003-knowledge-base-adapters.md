...
**留给 jason 的决定：**
- **AGPL 是否真的是红线？** 如果「可安装服务」走开源路线（自己也 AGPL），
  那 SiYuan（45k，本地优先 + 官方 MCP 生态最活跃）会是比 memos 更强的选择。
  这是商业模式问题，不是技术问题。
- **长文档需求有多重要？** 如果 `report` 类长产出是核心，组织档应换成
  `suitenumerique/docs`（MIT，面向长文档协作）而非 memos。
