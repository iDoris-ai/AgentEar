...
- 状态：**已拍板（jason 2026-09-26）**，六条决定见 §6 末尾；落地 = T3.5.6（v0.23.0，`src/qwen3.rs`）。
  原为「提议」（T3.5.6 第一步「调研」的产出）。
...
### 拍板结果（jason 2026-09-26）

| # | 问题 | 决定 |
|---|---|---|
| 1 | 运行时 | **先上 speech-swift**；GGUF 路线另开 **T3.5.7** 评准确率 |
| 2 | 常驻还是逐次 | **两种都提供**。默认**逐次**（小内存）；**菜单栏**手动切常驻，空闲超时自动退出（默认 600 s）。jason 要亲手对比两者 |
| 3 | 给哪几档 | **1.7B 与 0.6B 都给**（0.6B 给内存小的机器）；Qwen3 内默认 **0.6B**（「默认小内存」） |
| 4 | 下载源 | **直接从 Hugging Face 下，不打进我们的包**。落地时钉在固定 commit（`resolve/<commit>/`）+ 逐文件 sha256，挡住「第三方仓库随时改动」这个风险 |
| 5 | speech 版本 | **钉死**（`qwen3::SPEECH_VERSION` = v0.0.28 + tarball sha） |
| 6 | 调研留下的 611 MB 缓存 | 已由会话删除 |

⚠️ §4 推荐里的第 4 条（「只提供 1.7B」）与第 3 条推荐常驻为默认，**都被 jason 改了**——以上表为准。
落地中另有一条实测补充：speech 的断网沙箱**只能禁 TCP、不能禁 UDP**
（禁 UDP 会让它判缓存无效、转而重试下载），见 `src/qwen3.rs` `SANDBOX_PROFILE`。
