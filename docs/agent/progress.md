...
**更新时间**：2026-09-26
**当前分支**：main；本轮在 `feat/sidecar-port-fallback` 上（从 `fix/talk-first-turn` / PR #90 起）。
**最新已发布版本：v0.21.0**（PR #89，`895b1b8`；同日先发了 v0.20.1，PR #88，`ff657ae`）；
PR #90 = v0.21.1（对话模式首轮没回答）；**本 PR = v0.22.0**（端口自动避让 + 默认端口换 8796 + say 兜底，见下方）。
...
> 这条记下来，省得下次再试一遍。
## 本轮（2026-09-26，v0.23.0）：Qwen3-ASR 可选后端——设置窗口下载 + 菜单栏常驻（T3.5.6）

jason 2026-09-26 对 ADR-0010 §6 六问的拍板：**先上 speech-swift（版本钉死）**；
**常驻 / 逐次都给，默认逐次（小内存），菜单栏手动切常驻**；**1.7B / 0.6B 都给**；
**权重直接从 HF 下、不打进包**；缓存由会话清掉。

- 落地细节见 tasks.md T3.5.6；实测见 `benchmarks-asr-zh-en.md` §9：
  常驻热态 0.6B **0.09–0.35 s**、1.7B **0.19–0.69 s**（逐次 1.8–2.5 s）；
  常驻内存 0.6B ≈ **816 MiB**、1.7B ≈ **2.49 GiB**；空闲 600 s 自动退出。
- 本机实测过：从零下载运行时 + 0.6B（137 s）、1.7B 校验安装、断点续传、
  `--transcribe` 两档 × 两形态出字正确、同进程切换即时生效（`--asr-bench`）、
  SIGTERM / SIGINT / 菜单退出不留 speech-server 与 curl、kill -9 留下的孤儿下次被收掉、空闲回收。
- 顺带修了一个**泰语下载也有的老 bug**：被 kill 时 curl 子进程不死、继续写 `.part`。
- 顺带修了 T3.5.1 复现脚本：模型档位现在来自 config，原来那个「替换写死 `-m 1.7B`」的垫片
  会在新数据目录里**悄悄测成 0.6B**。
- **没测（要 jason 手测）**：设置窗口下拉 / 下载按钮 / 取消、菜单栏常驻开关的真实点击；
  守护进程真按键下两种形态的体感对比。
- 评审：Codex 额度到 2026-09-30，本 PR 走 Tier 2 本地自审一轮。

## 本轮（2026-09-26，v0.22.0）：边车端口自动避让 + 默认 TTS 端口 8796 + `say` 兜底（T3.4.18）

**jason 2026-09-26 拍板三条**（对 v0.21.1 的两个待决项）：① 默认 TTS 端口换掉（8765→**8796**）；
② 端口被别人占了要**自动发现、换一个端口拉起**；③ 边车起不来时**可以用 `say` 念一句**。
- 换端口只对「我们会拉起的」生效；**不改 config.json**，运行态记录在 `<数据目录>/run/talk-sidecars.json`
  （崩溃重启后按 pid 接回，不重复起 2–4 GB）。
- 本机实测全链路通过（见 T3.4.18）；**真按键双击没测**。
- **PR #92 被 PR-Daemon REQUEST_CHANGES**（进程死后不摘除登记 → pid 复用误杀 / 槽位占满 / 超时孤儿 / 按名字误伤），
  已按「pid 为键、观测到死亡即摘除」修掉，调用点变异 9 处全红，真 TTS 边车冒烟通过；等重审。
- ⚠️ 实测时测试守护进程截到了 jason 的一次按键（多录一段、多上屏一次）——记在 T3.4.18，下次实测要避开。
- 同一段误截的录音里 jason 说了一句「**你这个女生的声音不可爱呀，给那个卡哇伊**」——
  是对当时 `--say` 测试用的「女声」音色的反馈，**音色调整没做**，留给 jason 定。

## 本轮（2026-09-26，v0.21.1）：双击进对话模式后第一轮没回答（T3.4.17）

jason 装上 v0.21.0 实测：提示音单击、双击都正常，**但双击说完没有回答**。
查日志是两条 bug 叠加：① TTS 默认端口 8765 被 Chat On Steroids 占着（回 401），
被当成「边车没起」去拉起，撞端口立刻退出，日志没说原因；② 双击那一刻才拉边车，
第一轮说完 TTS 还没加载好，**即使端口没被占，第一轮也必然失败**。

- 当场处置：jason 本机 `config.json` 的 TTS 改到 **8766**（备份 `config.json.bak-20260926`），
  8766 上的 TTS 已由会话手工起着（不是守护进程拉起的，退出时守护进程不会收它）。
- 代码修法与验证见 tasks.md T3.4.17。**真按键链路没测**，要 jason 手测。
- 待拍板：默认端口换不换；边车不可用时要不要用 `say` 念一句。
- 教训已入记忆：发版前必须用新包真跑 `--diagnose/--say/--ask/--talk-turn` 冒烟；
  v0.20.1/v0.21.0 只跑了单测 + CI + 评审就发了。

## 本轮（2026-09-26，v0.21.0）：录音开始 / 结束提示音（T3.4.16）

jason 要的：按键后**听得见**话筒开了、停下时听到一个不同的声音；双击同样有。
单击 = 一声高 / 一声低，双击 = 两声高 / 两声低（`src/cue.rs`，`NSSound` 进程内播放）；
双击的第二下只**补一声**，凑成「嘟、嘟」。开始音在开麦**成功之后**才响，
结束音在关麦之后才响。设置窗口「录音提示音」，默认开。

- 实测：`play()` 返回 8.8–27.6ms（n=20）（预热后），闲置 20s 后约 48ms；数字混音 12 种组合 × 3 次
  转写与基线逐字相同（`docs/data/cue-asr-2026-09/`）。**真实声学路径、蓝牙延迟、
  真按键人耳都没测**，手测步骤在 `docs/releases/v0.21.0-notes.md`。
- 顺带发现：泰语（whisper）对纯静音本身会幻觉出字，与提示音无关，已记进 T3.4.16 的遗留。
- 评审：Codex 额度到 2026-09-30 才恢复，本 PR 走 Tier 2 本地自审一轮。

## 本轮（2026-09-26，v0.20.1）：崩溃后自动拉起 + 三条拍板入账

**jason 2026-09-26 拍板三件事**：
- **Q6 已决 = 接受风险 + 自动恢复**：录音中进程崩溃，丢掉当前那段可以接受，
  但要能自动重启、继续用。落地：`src/launch_agent.rs` 的 plist 从
  `KeepAlive = false` 改为 `KeepAlive = { SuccessfulExit = false }`；
  `src/sidecar.rs` 的 SIGTERM 处理改为以**退出码 0** 结束
  （否则用户 `kill` 它，launchd 10 秒后又拉回来）。菜单 Quit 走
  `NSApplication terminate`，按 Apple 文档是 `exit(0)`——**这条没在真 app 上实测**。
  - **launchd 语义实测**（临时 job + `/bin/sh` 替身，测完已 bootout）：
    `kill -9` → 拉起；`exit 1` → 拉起；**未捕获信号致死（HUP）→ 拉起**
    （这就是 SIGTERM 必须改成 exit 0 的原因）；trap 后 `exit 0` → 不拉；`exit 0` → 不拉。
    拉起间隔受 10 秒节流（按上次启动时间算）。
  - ⚠️ **边界**：只对 launchd 拉起的实例生效（Finder 双击打开的不管）；
    升级用户要「升级后第一次启动重写 plist → 下一次登录」才生效。
    启动期的永久性错误（如数据目录打不开）会变成每 10 秒一次的重启循环，日志可查。
- **Q4 已决 = 不换默认 ASR**：默认仍是随包的 SenseVoice；Qwen3-ASR 做成
  可配置 + 按需自动下载 → 新增 **T3.5.6**（`READY`，先调研再写代码）。
- **Q3 / T3.5.4 暂缓**：「方言那个先这样吧，不准备优化了，后续再说。」
  样本播过，**没有逐条结论**，不算验收通过。

另：T3.5.1 → `DONE`（PR #87，`7485bbe`）。
...
1. **jason 手测 T3.5.6**（v0.23.0）：设置窗口选 Qwen3 0.6B → 下载 → 自动切过去；
   菜单栏开关常驻，对比两种形态的体感。GGUF 评测另开 T3.5.7（`BACKLOG`）。
2. **T3.5.3 逐组件许可证表**——未确认许可的组件不得进默认方案（T3.5.6 的再分发也卡在这）。
3. **Agent24 接入**：ADR-0008 §5 六个问题仍未回答（Agent24 侧会话 2026-09-25 来问过现状）。
5. **T3.4.2 剩余出口项**：物理按键 → 声音停的端到端延迟实测（需要真机 + 人按键）；
   v0.19.0 / v0.20.x 升级后的人工验收（打断、设置窗口、开机自启、崩溃后被拉起）。
6. T3.4.15 打断修复的两条遗留（`BACKLOG`）、T3.5.2 / T3.5.5。

✅ 已移出：**T3.2.1 泰语 code-switch**（#82 → v0.19.0）；
**T3.4.2 的「播放期间按键打不断」**（#83 → v0.19.0）；
**Q5 打断方式**（已决，推键式）；**Q6**（已决，接受 + 自动拉起，v0.20.1）；
**Q4**（已决，不换默认 → T3.5.6）；**T3.5.1**（#87）；**T3.5.4**（暂缓）。
...
| # | 问题 | 阻塞谁 | 为什么我决定不了 |
|---|---|---|---|
| ~~Q3~~ | ⏸ **暂缓（jason 2026-09-26：不准备优化，后续再说）**。样本已播过、没有逐条结论——方言可用性**仍未验收** | ADR-0005、T3.5.4 | — |
| ~~Q4~~ | ✅ **已决（2026-09-26）：不换默认**，SenseVoice 随包；Qwen3-ASR 可配置 + 按需下载 → T3.5.6 | — | — |
| ~~Q5~~ | ✅ **已决：V1 打断方式 = 推键式长期终态**（2026-09-20 拍板，2026-09-25 jason 再次确认）。不补 VAD 自动打断 / 双讲 | — | — |
| ~~Q6~~ | ✅ **已决（2026-09-26）：接受丢失当前段 + 异常退出自动拉起**（v0.20.1，`KeepAlive.SuccessfulExit=false`）；不做录音中落盘、不缩短 300 秒上限 | — | — |
...
scripts/serve-tts.sh          # TTS，默认 127.0.0.1:8796（v0.22.0 前 8765）
...
- 2026-09-25 T3.5.1 中英 ASR 横比（PR #87，`7485bbe`，不发版）
- 2026-09-26 **v0.20.1**：LaunchAgent 异常退出自动拉起 + SIGTERM 以 0 退出；
  Q6 / Q4 已决、Q3 暂缓、新增 T3.5.6
