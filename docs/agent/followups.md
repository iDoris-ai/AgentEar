...
- [ ] FU-10 · B · src=T2.1.4 实测 · 2026-09-03 · 分批纠错后「我的妈 book」不再被纠正(分批前某次是对的):上下文变短导致,补了带空格 alias 也没用。是分批的已知代价,若将来影响体感可考虑重叠窗口(相邻批带一点上文)
- [ ] FU-11 · B · src=PR#T2.1.4-codex · 2026-09-03 · correct/label 的 HTTP 调用应抽象成可注入 transport,才能为「全成功/中间批失败/全失败/token 截断/批边界空白」写确定性测试。现在这些分支只能靠真实边车碰运气覆盖(codex Medium 4 + Low 1)
- [ ] FU-12 · B · src=PR#T2.1.3-codex · 2026-09-03 · last-good 不持久化:进程重启后缓存为空,若编辑器截断文件后崩溃/断电,启动加载只能用内置默认表。可考虑每次成功解析后原子更新 terms.json.bak,主文件坏且无内存缓存时读备份(codex Low 5)
- [ ] FU-13 · B · src=T2.2.5 · 2026-09-03 · 标签分类唯一稳定判错:「要是能用语音直接建任务就好了」被句中「建任务」三字带走判成 task。定义已说清「承诺要做了吗」,是模型能力问题——修法是 few-shot 补一条虚拟语气的反例,不要改定义去迁就
- [ ] FU-14 · B · src=T2.2.5 · 2026-09-03 · spike/m2_bench.py 报 18/18 而生产路径 17/18,差异稳定复现但根因未定位(已排除标签顺序、期望值、规则语义)。让基准直接调用生产分类路径可一并消掉这个疑点(与 FU-6 同源)
- [ ] FU-15 · B · src=T3.1.2 · 2026-09-03 · env -u HF_ENDPOINT 是本机 huggingface_hub 报 LocalEntryNotFoundError 的通用绕法(第三次遇到)。build-thai-model.sh 和 setup-llm.sh 现在用的是 curl 逐文件,可以简化成这个
