...
pub enum AsrBackend {
    ...
    ///
    /// ⚠️ **这个后端超出既有的 ASR 资源标准，尚未拍板。**
    /// 实测 Qwen3-ASR 1.7B 单次峰值 RSS **2.43 GiB**，而 `CLAUDE.md` 与 ADR-0001
    /// 对 ASR 侧的要求仍是 **≤2 GiB**（M2 的预算放宽「只针对 LLM 这一项」）。
    ///
    /// **「可选而非默认」只能避免升级回归，不构成对资源标准的豁免。**
    /// 启用它之前需要 jason 明确二选一：
    /// 为高资源档单独立预算，或者换一个满足 ≤2 GiB 的配置并实测
    ///（0.6B 约 1.2 GiB 但三项指标都退，见 benchmarks-m3.md §2.1.1）。
    /// 见 `docs/agent/tasks.md` T3.4.6。
    SpeechSwift,
}
