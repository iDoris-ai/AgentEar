...
///
/// ⚠️ **这个数是照 macOS 自己的双击判定来的，不要凭手感调小。**
/// 系统默认 `NSEvent.doubleClickInterval` 是 **500ms**，用户的肌肉记忆就是按它训练的；
/// 比它更紧等于要求用户「比双击应用程序图标还快」，而失败的症状是
/// **静默的**——判成两次单击 → 开一段、又停一段 → 「录音过短丢弃」，
/// 用户只会觉得「双击没反应」，看不出是手感问题。
///
/// 实测踩到过（2026-09-14，jason 第一次试）：他两下相隔 **1004ms**，
/// 而当时的窗口是 350ms —— 连三下都没用。所以这里对齐系统惯例。
///
/// 代价说清楚：窗口越宽，越容易把「点一下、想一下、再点一下」吞成双击。
/// 500ms 是系统已经在用的折中点，不另发明。
pub const DOUBLE_TAP_MAX_MS: u64 = 500;
...
mod tests {
    ...
    fn double_tap_window_is_inclusive_at_the_edge() {
        ...
        // **真实失败样本回归**：jason 2026-09-14 第一次试双击，两下相隔 1004ms，
        // 当时窗口 350ms → 判成两次单击 → 「录音过短丢弃」→ 他的感受是「双击没反应」。
        // 这条钉住「1004ms 仍然算两次单击」这个事实，免得以后有人把窗口放到 1 秒去
        // 迁就它——1 秒的窗口会把正常的两段录音吞掉。
        assert_eq!(
            classify_tap(2004, Some(1000)),
            TapKind::Single,
            "相隔 1004ms 是两次独立单击；要双击得按得比 500ms 更快"
        );
