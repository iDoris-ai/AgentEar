...
- 修法：`_load_ref` **返回路径字符串**，让模型按自己的契约解码 + 重采样。
  ⚠️ **这句原话错了，2026-09-19 实测更正**：「代价是每次合成重读一次参考 wav，
  相对合成时长可忽略」——把成本理解成了**文件 I/O**，实际成本是**计算**：
  每次调用都要把参考文本（465 字）重新过一遍 `base_lm` 的初始前向，
  实测 **2.8–2.9 秒**（同一进程内连打 5 次，同文不同文都一样，不会因重复调用而降），
  是短句合成里最大的单项开销，不是可忽略的量级。**详见 ADR-0009**，
  已排进 T3.4.13 评估用缓存消掉这笔重复计算。
