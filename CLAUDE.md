...
## 当前状态：**v0.15.0 —— 修掉克隆音高偏八度（参考音频采样率在边车里丢了）**；M1/M2 已发布；**M3 通话链路已跑通，AEC / 自动打断未做**
...
- 修法：`_load_ref` **返回路径字符串**，让模型按自己的契约解码 + 重采样。
  代价是每次合成重读一次参考 wav（1–9 MB），相对 2.4–6 s 的合成可忽略。
...
- 菜单 **说话** 三栏：**语系**（12 项，含粤语/四川/河南/山东/东北/天津）、
  **语气**（4 项）、**音色**（**扫目录，文件名就是菜单里显示的名字**）。
- 所以「音色名字」= 参考音频的文件名。jason 机器上已重命名为 **`男声`**
  （他自己的声音，142.4 Hz）与 **`女声`**，其余两条移进 `voices/archive/`
  （边车 `glob("*.wav")` 不递归，所以归档即从菜单消失）。
- **音色 × 语系是任意组合**：参考决定音色、instruct 决定口音/语气
  （官方叫 Controllable Voice Cloning）。实测「男声 + 四川话」音色跟住了
  参考（153.8 vs 144.4 Hz）。
